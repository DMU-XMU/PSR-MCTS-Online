...
public class MCTS {
    ...
    public MCTS(APSR psrModel, double explorationConstant, int numSimulations, int expandCount, int maxDepth, boolean reuseTree, ASimulator simulator) {
    ...
    public int GreedyUCB(Node.VNODE vnode,boolean ucb){
        ...
        for (int action=0;action<actNum;action++){
            double q;
            int n;
            Node.QNODE qnode=vnode.Child(action);
