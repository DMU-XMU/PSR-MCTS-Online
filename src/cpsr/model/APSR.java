...
{
	protected PredictionVector pv, initialPv,initialPvUpdate;
	...
	protected HashMap<ActionObservation, DoubleMatrix> aoMatsNum;
	...
	public void build()
	{
		...
		initialPvUpdate=new PredictionVector(pv.getVector().dup());
	...
	public void updateSaving(ActionObservation ao)
	{
		DoubleMatrix tempMao = aoMats.get(ao);
		DoubleMatrix numerator, denominator;
		try
		{
			numerator = (tempMao.mmul(initialPvUpdate.getVector()));
		}
		catch(NullPointerException ex)
		{
			System.err.print("Unknown observation: no update performed(saving)");
			return;
		}
		denominator = (mInf.getVector().transpose()).mmul(numerator);
		...
		try
		{
			if(!(denominator.get(0, 0) == 0.0)) initialPvUpdate = new PredictionVector(numerator.mul((1.0/denominator.get(0,0))));
		} catch (PVException e)
		{
			e.printStackTrace();
			System.exit(0);
	...
	public void setPvToCurrentState(){
		try
		{
			pv = new PredictionVector(initialPvUpdate.getVector().dup());
		}
	...
	public void resetPvToInitial(){
		try
		{
			pv = new PredictionVector(initialPv.getVector().dup());
			initialPvUpdate=new PredictionVector(initialPv.getVector().dup());
	...
	@Override
	public HashSet<ActionObservation> getActionSetByact(Action act){
		return this.trainData.getValidActionObservationSetByAct(act);
	}
	...
	public DoubleMatrix getAOMat(ActionObservation actob) throws PSRRuntimeException
	{
		...
		if(mao == null)
		{
			System.out.println("act"+actob.getAction().getID()+"obs"+actob.getObservation().getID());
