...
public class TrainingDataSet extends DataSet implements Serializable
{
	...
	//
	//protected HashSet<ActionObservation> actObsByAct;
	...
	public void addRunDataForTraining(ArrayList<ActionObservation> actobs)
	{
		...
		//for(int i = Math.max(actobs.size() - maxTestLength, 0); i < actobs.size(); i++)
		{
	...
	public HashSet<ActionObservation> getValidActionObservationSetByAct(Action act) {
		HashSet<ActionObservation> aoSet=new HashSet<>();
		for (ActionObservation ao:validActObs
			 ) {
			//System.out.print(ao.getAction().getID());
			//System.out.println(ao.getObservation().getID());
			if (ao.getAction().equals(act)){
				aoSet.add(ao);
			}
